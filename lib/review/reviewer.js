...
export function buildReviewPrompt(ctx) {
    ...
    if (ctx.humanOverrideId !== undefined) {
        lines.push('');
        lines.push(`HUMAN OVERRIDE ${buildMarker('ap/override', ctx.humanOverrideId)}: the user recently approved this tool family once. Treat this as STRONG EVIDENCE of intent — you may still deny if the specific action looks unrelated or dangerous.`);
    }
...
export function parseVerdict(raw, stopReason) {
    ...
    if (reason.length === 0)
        throw new Error('empty verdict reason');
 ...
 */
export function resolvePolicy(toolName, reason, rules, overrides, fallbackDefault) {
    ...
    if (overrideKey !== undefined)
        return overrides[overrideKey] ?? fallbackDefault;
