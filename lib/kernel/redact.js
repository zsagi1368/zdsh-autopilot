...
export function redactedBulkPlaceholder(key, length) {
    return `[redacted-${key}:${length}-chars]`;
}
function normalizeKey(key) {
    return key.toLowerCase().replace(/[-_.]/g, '');
}
const NORMALIZED_SECRET_MARKERS = SECRET_KEY_MARKERS.map(normalizeKey);
const NORMALIZED_BULK_NAMES = BULK_KEY_NAMES.map(normalizeKey);
function isSecretKey(key) {
    const norm = normalizeKey(key);
    return NORMALIZED_SECRET_MARKERS.some(marker => norm.includes(marker));
}
function isBulkKey(key) {
    return NORMALIZED_BULK_NAMES.includes(normalizeKey(key));
}
export function redactString(text, profile) {
    let out = text;
    const patterns = profile === 'strict' ? STRICT_TEXT_PATTERNS : TOKEN_PATTERNS;
    for (const { re } of patterns) {
        out = out.replace(re, REDACTED_SECRET);
    }
    if (out.length > REDACT_CAPS.scalarChars)
        out = `${out.slice(0, REDACT_CAPS.scalarChars)}…`;
    return out;
}
export function redact(value, profile = 'standard') {
    function walk(node, depth) {
        if (typeof node === 'string')
            return redactString(node, profile);
        if (typeof node === 'number' || typeof node === 'boolean' || node === null)
            return node;
        if (node === undefined)
            return null;
        if (depth >= REDACT_CAPS.depth)
