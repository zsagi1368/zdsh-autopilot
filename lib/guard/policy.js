 ...
 */
export function hardDeny(toolName, argsJson, roots) {
    ...
    if (destructiveHead || /\b(rm|del|remove)\b/i.test(argsJson)) {
        for (const p of argPaths) {
            ...
            if (normalizePath(p).driveRelativeAmbiguity &&
...
function extractPathLikeArgs(argsJson) {
    const out = [];
    ...
    if (scanEmbedded || out.length === 0) {
        ...
        for (const m of argsJson.matchAll(/[A-Za-z]:[\\/][^\s"']*/g))
            out.push(m[0]);
...
export function assessTool(toolName, argsJson, roots, hasArtifact) {
