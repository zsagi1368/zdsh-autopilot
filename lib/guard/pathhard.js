...
export function isWithin(root, candidate) {
    ...
    if (!c.isAbsolute)
        return false; // relative targets are judged by the caller
    if (r.isAbsolute && r.normalized.slice(0, 2) !== c.normalized.slice(0, 2))
        return false;
    const rel = relativePosix(r.normalized, c.normalized);
    if (rel === '')
        return true; // identical path counts as contained
...
export function isCriticalPath(roots, candidate) {
    ...
    if (isWithin(roots.dshHomePath, c.normalized) ||
...
export function isCredentialTree(candidate) {
    const c = normalizePath(candidate);
...
export function isProtectedProjectMeta(workspaceRoot, candidate) {
    const c = normalizePath(candidate);
    if (!isWithin(workspaceRoot, c.normalized))
        return false;
