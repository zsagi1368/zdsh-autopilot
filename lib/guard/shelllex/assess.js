...
function assessWordSegment(segment, ctx) {
    ...
    for (const prefix of ROUTINE_PREFIXES) {
        if (lower.startsWith(prefix))
            return { decision: 'allow', reason: 'routine build/test/vcs work inside the sandbox' };
    }
...
export function assessCommandLine(shell, line, artifacts, roots) {
    ...
    let worst = { decision: 'allow', reason: 'sandbox-confined' };
    const rank = { allow: 0, classify: 1, deny: 2 };
    for (const segment of decomposition.segments) {
        const assessment = segment.kind === 'opaque'
            ? opaqueAssessment(segment.text, roots)
            : assessWordSegment(segment, { artifacts, roots });
        if (rank[assessment.decision] > rank[worst.decision])
            worst = assessment;
