...
function mount(ctx) {
        ...
        void (async () => {
            try {
                ...
                if (outcome === 'delegate' || outcome === 'unavailable') {
                    const grantVerdict = flags.guard && req.callId !== undefined ? guard.handleApprovalRequest({ callId: req.callId, toolName: req.toolName ?? '' }) : undefined;
    ...
    webServer?.register({
        ...
        handler: async (req) => {
            const bridgeReq = req;
            const authorize = (payloadText) => authorizeAction(bridgeReq, payloadText, bridgeToken);
...
function authorizeAction(req, payloadText, expectedToken) {
    ...
    if (origin !== undefined && origin.length > 0)
        return sameOrigin(headers['host'], origin);
